...
def complex_model(X,y,is_training):
    ...
    hidden_flat  = tf.reshape(p1,[-1,5408])
    hidden_out_1 = tf.matmul(hidden_flat,W1) + b1
    hidden_out_2 = tf.nn.relu(hidden_out_1)
    
    y_out = tf.matmul(hidden_out_2, W2) + b2
    return y_out
...
#optimizer = tf.train.RMSPropOptimizer(starter_learning_rate)
...
# After a few iterations, I managed to get >70% accuracy. The thing which helped me the most was using the exponential learning rate decay with the Gradient Descent Optimizer.
# 
# Here are things I played around with
# 
# - Filter size: Played around using 3x3 instead of 7x7..Didnt seem to help much.
# - Number of filters: Kept it fixed at 32
# - Pooling vs Strided Convolution: Use max pooling . Didnt use strided convolutions
# - Batch normalization: Added batch normalization after convolution layers. Did not use vanilla batch normalization after affine layers.
# - Network architecture: Used the same network defined above
# - Use Learning Rate Decay: This helped
# - Global Average Pooling: Did not try this
# - Regularization: Tried adding Dropout. But ran into issues. 
# 
